...
import sys
import jsonlines
from docker_adapter import docker_app, get_docker_client
from swebench.harness import run_evaluation

app = modal.App("distributed-swebench")
app.include(docker_app)

custom_image = Image.debian_slim(python_version="3.10").poetry_install_from_file(
...
def run_swebench(predictions_path, run_id, workers, shard):
    # Monkey-patch the docker client creation in SWE-bench
    import swebench.harness.run_evaluation

    swebench.harness.run_evaluation.docker.from_env = get_docker_client
    ...
    try:
        swebench_results = run_evaluation.main(
...
def main(
    predictions_path: str, run_id: str, num_shards: int = 10, workers_per_shard: int = 4
):
...
def create_shards(predictions_path, num_shards):
    ...
    for i in range(num_shards):
        ...
        shard = {"shard_id": i, "data": data[start:end]}
        shards.append(shard)
    return shards
